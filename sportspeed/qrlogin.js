...
function getqrpic(force){
	...
	if(qrsig!=null && qrimg!=null && force==false){
		...
		var getvcurl='https://tool.clwl.online/speed/sportspeed/login.php?do=getqrpic&r='+Math.random(1);
...
function qrlogin(){
	...
	var url = 'https://tool.clwl.online/speed/sportspeed/login.php?do=qrlogin&qrsig='+decodeURIComponent(qrsig)+'&r='+Math.random(1);
	$.get(url, function(d) {
		if(d.saveOK ==0){
			$('#login').html('<div class="alert alert-success">提交至服务器成功！</div>');
			$('#qrimg').hide();
			$('#submit').hide();
