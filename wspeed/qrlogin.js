...
function getqrpic(force){
	...
	if(qrsig!=null && qrimg!=null && force==false){
		...
		var getvcurl='https://tool.clwl.online/speed/wspeed/login.php?do=getqrpic&r='+Math.random(1);
...
function qrlogin(){
	...
	var url = 'https://tool.clwl.online/speed/wspeed/login.php?do=qrlogin&qrsig='+decodeURIComponent(qrsig)+'&r='+Math.random(1);
	$.get(url, function(d) {
		if(d.saveOK ==0){
			$('#login').html('<div class="alert alert-success">登录成功！</div>');
			$('#qrimg').hide();
			$('#submit').hide();
			$('#login').attr("data-lock", "true");
			$.ajax({
				url:"https://tool.clwl.online/speed/wspeed/login.php?do=speed",
				type:"post",
				dateType:"json",
				data:d.cookie,
				success:function(data){
					$('#login').html('<div class="alert alert-success">加速成功</div>');
				},
				error:function(){
					$('#login').html('<div class="alert alert-danger">加速失败！</div>');
				}
			});
			cleartime();
		}else if(d.saveOK ==1){
